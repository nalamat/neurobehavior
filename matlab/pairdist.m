...
% Modified by BNB to bypass some memory issues with handling larger arrays
% that was present in the two lines commented out above.  Essentially this
% avoids allocating a number of large temporaries by using a for loop.
XT = X';
for i = 1:size(normsqrY, 1)
    dists(:,i) = dists(:,i) + normsqrY(i) - (2 * Y(i,:) * XT)';
end
