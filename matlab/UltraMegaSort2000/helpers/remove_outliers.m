...
%
    ...
    if ~isfield(spikes.info,'outliers')
        ...
        % Added by BNB on 2012.03.01
        if isfield(spikes.params, 'extra_fields')
            o = spikes.info.outliers;
            for i = 1:length(spikes.params.extra_fields),
                fieldname = spikes.params.extra_fields{i};
                o = setfield(o, fieldname, []);
            end
            spikes.info.outliers = o;
        end
    end

    % BNB edit -- fixed inconsistent indentation for lines below this point on
    % 2012.03.01
    ...
    if isfield(spikes.params, 'extra_fields'),
        for i = 1:length(spikes.params.extra_fields),
            fieldname = spikes.params.extra_fields{i};
            r_fielddata = getfield(spikes, fieldname);
            o_fielddata = getfield(spikes.info.outliers, fieldname);
            o_fielddata = [o_fielddata, r_fielddata(bad)];
            r_fielddata(bad) = [];
            spikes = setfield(spikes, fieldname, r_fielddata);
            spikes.info.outliers = setfield(spikes.info.outliers, fieldname, o_fielddata);
        end
    end
