...
# "launchers.positive_cmr" (i.e. the "name" of the module as seen by Python).
import logging
...
class Controller(
    ...
    def start_experiment(self, info):    
        ...
        # AbstraactPositiveExperiment.start_experiment() to finish the sequence
        # of operations that are required to run the experiment.

        # Generate a random seed from the time in seconds since start of the
        # Unix epoch
    ...
    def set_hw_att(self, atten):
        # The RPvds circuit has a toggle to send the waveform to one speaker or the other (the inactive speaker essentially recieves a string of 0's), so we will simply set both speakers to the same attenuation to avoid the "click" in between every trial that occurs when the hardware attenuators are set.
    ...
    def set_nogo_filename(self, filename):
        ...
        # Use loadtxt because it convers the data to numeric values rather than loading as a string, then reverse the array and convert it to a list.  We convert to a list because this allows us to take advantage of the pop() method.  Numpy arrays do not have this feature, so we'd have to rewrite other parts of the code if we kept the data structure as an array rather than converting it to a list.In Numpy, extra dimensions are implied in contrast with matlab where you specifically have to provide the : to include them.
    ...
    def trigger_next(self):
        # This function is *required* to be called.  This basically calls the logic (defined in AbstractExperimentController) which clears the current value of all parameters and recomputes them (this is important in between trials).
        self.invalidate_context()
        # This must be called before every trial to load (or evaluate if the parameter is an expression) the values of each parameter.  Note this method will also check to see if the value of a parameter has changed since the last trial.  If so, the corresponding set_parametername method will be called with the new value as an argument.
        ...
        F, E, FC, TL, ML, TokenNo, TargetNo = settings
        ...
        # Scale waveforms so that we get desired stimulus level assuming 0 dB of attenuation
        ...
        # Set the flag in the RPvds circuit that indicates which output to send the waveform to
        if speaker == 'primary':
        ...
        # Be sure that the trial type is added to the list of context variables that will be saved to the trial_log file.
        self.set_current_value('ttype', ttype)

        # Boolean flag in the circuit that indicates whether or not the current trial is a go.  This ensures that a reward is not delivered on NOGO trials.
        if ttype.startswith('GO'):
            self.iface_behavior.set_tag('go?', 1)
        else:
            self.iface_behavior.set_tag('go?', 0)
        
        # This is a "handshake" that lets the RPvds circuit know that we are done with preparations for the next trial (e.g. uploading the stimulus waveform).  The RPvds circuit will not proceed with the next trial until it receives a "SoftTrig1" (in TDT parlance)
...
class Paradigm(
    ...
    nogo_filename = File(context=True, log=True, label='NOGO filename')
    go_filename = File(context=True, log=True, label='GO filename')
