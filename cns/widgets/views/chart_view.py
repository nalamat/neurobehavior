...
from enthought.traits.api import Str, Int, Float, Trait, Any, Callable, Instance, HasTraits, DelegatesTo
import numpy as np
...
class DynamicBarPlotView(SingleBarPlotView):
    ...
    def _source_changed(self, old, new):
        if old is not None:
            old.on_trait_change(self.update_labels, "updated", remove=True)
            old.on_trait_change(self.update_data, "updated", remove=True)
        if new is not None:
            new.on_trait_change(self.update_labels, "updated")
