...
class InterpolatedLineSegmentTool(AbstractOverlay):

    # The component that this tool overlays
    component = Instance(Component)
    
    # The current line segment being drawn.
    line = Instance(Line, kw=dict(close_path=False))
    ...
    def _get_screen_points(self):
        points = list(self.component.map_screen(array(self.points)))
    ...
    def __init__(self, component=None, **kwtraits):
        ...
        super(InterpolatedLineSegmentTool, self).__init__(**kwtraits)
    ...
    def normal_left_down(self, event):
        ...
        if over is not None:
            if event.control_down:
                ...
                self.line.points = self.screen_points
                self.request_redraw()
            else:
            ...
            x, y = self._map_data((event.x, event.y))
