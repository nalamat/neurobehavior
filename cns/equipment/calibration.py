...
from enthought.traits.api import HasTraits, Str, Float, Any, Property, Instance, Array, Tuple, Enum
...
class Microphone(HasTraits):
    ...
    manufacturer = Str
    ID = Any
    nominal_sens = Float(units='Vrms/Pa')
...
class Calibration(HasTraits):
    ...
    #microphone = Instance(Microphone, editor=InstanceEditor(values=MICROPHONE_OPTIONS))
    microphone  = Enum(MICROPHONE_OPTIONS)
    standard    = Enum(STANDARD_OPTIONS)
...
def ref_cal(duration=1, averages=2, standard=None, microphone=None):
    '''Calibrates measuring microphone against a known reference (e.g. a
    pistonphone).  Typically this is the B&K microphone, but certainly could be
    any microphone that is felt to produce a flat frequency response across the
    range of frequencies of interest.
