...
class DSPBuffer(BlockBuffer):
    ...
    def _simple_read(self, offset, length):
        ...
        if length == 0:
            ...
            #print offset, length, type_lookup[self.src_type]
            #data = self.ReadTagVEX(offset, length, 'I32', type_lookup[self.src_type], 1)
            #data = self.dsp.ReadTagVEX('mc16', 0, 4, 'I32', 'I16', 4)
            data = self.ReadTagV(offset, length)
            #print np.array(data, dtype=np.float32)/self.sf
    ...
    def _c_read(self, offset, length):
        ...
        #data = self._simple_read(offset*self.c_factor, length*self.c_factor)/self.sf
        #print data
        return data.astype(np.float32)/self.sf
    ...
    def _mc_dec_read(self, offset, length):
        #data = self._mc_c_read(offset, length)
