...
class DSPBuffer(BlockBuffer):
    ...
    def _simple_read(self, offset, length):
        ...
        if length == 0:
            ...
            # The RZ5 and RX6 use float32 as the native datatype.  Numpy appears to use float64 by default.
            return np.array(data, dtype=np.float32).ravel()
        
    def _c_read(self, offset, length):
        data = self._simple_read(offset*self.c_factor, length*self.c_factor)
        return data/self.sf
