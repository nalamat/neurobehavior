...
from cns.data.persistence import append_node, add_or_update_object
...
from enthought.traits.ui.api import HGroup, spring, Item, View, Handler, UIInfo
import logging
import numpy as np
import time
...
class AversiveController(ExperimentController):
    ...
    def init_equipment(self, info):
        ...
        #self.circuit = equipment.dsp().load('aversive-behavior', 'RX6')
        self.circuit = equipment.dsp().load('aversive-behavior-oldbox', 'RX6')
    ...
    def configure_circuit(self, circuit, paradigm):
        ...
        # Set sampling frequency of DSP so that the appropriate waveform is
        # generated.
        paradigm.signal_warn.fs = circuit.fs
        paradigm.signal_safe.fs = circuit.fs

        circuit.lick_th = paradigm.lick_th
        circuit.min_int_dur = len(paradigm.signal_safe)

        circuit.trial_dur = len(paradigm.signal_warn)
        circuit.shock_del = circuit.sec_to_samples(paradigm.shock_delay)
        #circuit.shock_dur = circuit.sec_to_samples(paradigm.shock_duration)

        if circuit.fs < paradigm.requested_lick_fs:
            raise ValueError, 'Requested sampling rate for spout ' + \
                    'contact is greater than the DSP clock frequency.'
        else:
            # To achieve requested sampling rate for spout contact, we need
            ...
            samples = int(circuit.fs / paradigm.requested_lick_fs)
            circuit.lick_nPer = samples
            paradigm.actual_lick_fs = circuit.fs / samples

        circuit.open('int_buf', 'w', len(paradigm.signal_safe))
        circuit.int_buf.write(paradigm.signal_safe)
        circuit.open('contact_buf', 'r', src_type=np.float32,
                                         dest_type=np.float32,
                                         channels=4,
                                         compression='decimate',
                                         sf=1)
        circuit.pause_state = True
    ...
    def initialize_data(self, model):
        ...
        model.data = AversiveData(contact_fs=model.paradigm.actual_lick_fs,
    ...
    def run(self):
        ...
        try:
            ...
            self.circuit.trial_buf = self.current.signal_warn
    ...
    def remind(self, info=None):
        ...
        self.circuit.trial_buf = self.current.signal_remind
        self.circuit.shock_level = self.current.shock_remind
        self.circuit.trigger(2) # Go into warning on next trial
        self.circuit.pause_state = False # Everything's ready. GO!

    def pause(self, info=None):
        self.state = 'paused'
        self.circuit.pause_state = True

    def resume(self, info=None):
        self.state = 'running'
        self.circuit.pause_state = False
    ...
    def task_update_data(self):
        data = self.circuit.contact_buf.next()
    ...
    def task_monitor_circuit(self):
        if self.circuit.idx > self.current.idx:
            self.current.idx += 1
            ts = self.circuit.ts_trial_start / self.circuit.fs

            # Process "reminder" signals
            if self.state == 'manual':
                self.pause()
                self.model.data.update(ts, self.current.par_remind, 
                                       self.current.shock_remind, 'remind')
                self.circuit.trial_buf = self.current.signal_warn
                self.circuit.shock_level = self.current.shock_level
                return

            # Warning was just presented.
            self.current.trial += 1     # reminders do not count
            if self.current.trial == self.current.safe_trials + 2:
                log.debug('processing warning trial')
                self.model.data.update(ts, self.current.par,
                                       self.current.shock_warn, 'warn')
                self.current.next()
                self.circuit.trial_buf = self.current.signal_warn
                self.circuit.shock_level = self.current.shock_warn
    ...
    def apply(self):
        ...
        for (object, name), value in self.pending_changes.items():
            ...
            if name == 'lick_th':
                self.circuit.lick_th = value
                self.model.data.log(self.circuit.ts, name, '%f' % value)
            elif name == 'par_order':
                reset_settings = True
                self.model.data.log(self.circuit.ts, name, value)
            elif name == 'pars':
                reset_settings = True
                self.model.data.log(self.circuit.ts, name, value)
