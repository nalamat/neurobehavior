...
class ParameterInterface(wx.Panel):
	def __init__(self, parent, title="", soundManager=None):
	...
	def onAddAxis(self, event):
		widgetToAdd = AxisWidget.AxisWidget(self, wx.ID_ANY, title="Widget", yAxis=Axis.Axis(), soundManager = self.soundManager)
		self.placeWidget(widgetToAdd)
				
	def onAddSignal(self, event):
		widgetToAdd = GraphWidget.GraphWidget(self, wx.ID_ANY, title="Signal", soundManager=self.soundManager)
		self.placeWidget(widgetToAdd)
