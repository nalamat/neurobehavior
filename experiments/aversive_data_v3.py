...
class RawAversiveData_v0_2(AbstractExperimentData):
    ...
    #    print self.pars, self.par_safe_count, self.par_warn_count
    ...
    def _get_par_info(self):
        data = {
                ...
                'par_safe_count':   self.par_safe_count,
                'par_warn_count':   self.par_warn_count,
                }
    ...
    def _get_safe_par_mask(self):
        return self._get_par_mask() & (self.ttype_seq == 'safe')

    @cached_property
    def _get_warn_par_mask(self):
        return self._get_par_mask() & (self.ttype_seq == 'warn')
    ...
    def _get_pars(self):
        #seq = np.take(self.par_seq, self.warn_indices, axis=0)
    ...
    * global_fa_frac
    ...
    def _get_par_info(self):
        data = {
                ...
                'dprime_nonglobal': self.par_dprime_nonglobal,
                'dprime_global':    self.par_dprime_global,
                }
...
class AnalyzedAversiveData(BaseAnalyzedAversiveData):
    ...
    def _get_masked_trial_log(self):
        ...
        warn_indices = np.flatnonzero(log['ttype'] == 'warn')
    ...
    par_seq = Property(depends_on='masked_trial_log')
    ttype_seq = Property(depends_on='masked_trial_log') 

    def _get_par_seq(self):
        try:
            arr = np.empty(len(self.masked_trial_log), dtype=object)
            arr[:] = zip(*[self.masked_trial_log[p] for p in self.data.parameters])
            return arr
        except BaseException, e:
            return np.array([])
    ...
    def _compute_contact_scores(self, trial_log):
        ...
        if self.contact_reference == 'trial start':
            timestamps = trial_log['ts_start']
        else:
            timestamps = trial_log['ts_end']
    ...
    def _get_global_fa_frac(self):
        mask = self.masked_trial_log['ttype'] == 'safe'
    ...
    def _get_warn_trial_count(self):
        return (self.masked_trial_log['ttype'] == 'warn').sum()
